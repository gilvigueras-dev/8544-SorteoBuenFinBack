...
APP_NAME="sbf-api-productos"
BUILD_CONFIG="Release"
PUBLISH_DIR="./publish"
ZIP_FILE="./publish/sbf-api-productos-publish.zip"
...
if [ $? -eq 0 ]; then
    ...
    echo -e "${GREEN}🌐 URL: https://$APP_NAME-hfg0hvcvfshcfhhf.eastus2-01.azurewebsites.net${NC}"
    
    # Preguntar si quiere abrir la URL
    read -p "¿Quieres abrir la aplicación en el navegador? (y/n): " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        open "https://$APP_NAME-hfg0hvcvfshcfhhf.eastus2-01.azurewebsites.net"
    fi
else
