...
namespace SAT_API.Api.Controllers
{
    ...
    public class ProductosController : ControllerBase
    {
        ...
        public async Task<ActionResult<ApiResponse<List<ProductResponseDto>>>> GetProductos([FromRoute] ProductRequestDto request)
        {
            ...
            if (result.IsSuccess)
            {
                return Ok(ApiResponse<List<ProductResponseDto>>.SuccessResponse(result.Value!, result.Message + "Publicado desde VS Code"));
            }
        ...
        [HttpGet("ControlNumbersUrl/{request}")]
        ...
        /// <summary>
        /// Obtiene el archivo de cifras de control.
        /// </summary>
        /// <param name="request">Parámetros de entrada para obtener el archivo de cifras de control en base a la clase <seealso cref="ControlNumbersRequestDto"/></param>
        /// <returns>El archivo de cifras de control.</returns>
        [HttpGet("ControlNumbersFile/{request}")]
        [ProducesResponseType(typeof(ApiResponse<ControlNumbersFileResponseDto?>), StatusCodes.Status200OK)]
        [ProducesResponseType(typeof(ApiResponse<ControlNumbersFileResponseDto?>), StatusCodes.Status400BadRequest)]
        [ProducesResponseType(typeof(ApiResponse<ControlNumbersFileResponseDto?>), StatusCodes.Status404NotFound)]
        [ProducesResponseType(typeof(ApiResponse<ControlNumbersFileResponseDto?>), StatusCodes.Status500InternalServerError)]
        public async Task<ActionResult<ApiResponse<List<ControlNumbersFileResponseDto>>>> ControlNumbersFile([FromRoute] ControlNumbersRequestDto request)
        {
            var result = await _productsService.ControlNumbersFile(request);
            if (result.IsSuccess)
            {
                return Ok(ApiResponse<List<ControlNumbersFileResponseDto>>.SuccessResponse(result.Value ?? new List<ControlNumbersFileResponseDto>(), result.Message));
            }

            return Ok(ApiResponse<List<ControlNumbersFileResponseDto>>.ErrorResponse(result.Message, result.Errors));
        }
    }
}