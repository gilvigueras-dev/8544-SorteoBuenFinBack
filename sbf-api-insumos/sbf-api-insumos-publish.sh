...
APP_NAME="sbf-api-insumos"
BUILD_CONFIG="Release"
PUBLISH_DIR="./publish"
ZIP_FILE="./publish/sbf-api-insumos-publish.zip"
...
if [ $? -eq 0 ]; then
    ...
    echo -e "${GREEN}🌐 URL: https://$APP_NAME-bugefwffe4gwbqbz.eastus2-01.azurewebsites.net${NC}"
    
    # Preguntar si quiere abrir la URL
    read -p "¿Quieres abrir la aplicación en el navegador? (y/n): " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        open "https://$APP_NAME-bugefwffe4gwbqbz.eastus2-01.azurewebsites.net"
    fi
else
